...
function orderAlphabetically(str) {
  ...
  convertStrToArr.sort();
  convertStrToArr = convertStrToArr.toString().replaceAll(',', '');
  return convertStrToArr;
}
...
function countVowels(str) {
  const vowelCounter = str.match(/[aeiouyAEIOUY]/gi);
...
function isPalindrome(str) {
  ...
  strToLowerCase = strToLowerCase.toLowerCase();
  let newStr = '';
  for (let i = str.length - 1; i >= 0; i -= 1) {
    newStr += strToLowerCase[i];
  }
  strToLowerCase = strToLowerCase.replaceAll(
    /[!"#$%&'()*+,./:;<=>?@_`{|}~-\s]/g,
    ''
  );
  newStr = newStr.replaceAll(/[!"#$%&'()*+,./:;<=>?@_`{|}~-\s]/g, '');
  if (strToLowerCase === newStr) {
    return true;
  }
  return false;
}
...
function reverseWords(str) {
  ...
  for (let j = 0; j < newArr.length; j += 1) {
    ...
    for (let i = newArr[j].length - 1; i >= 0; i -= 1) {
      arr += newArr[j][i];
    }
  }
  return arr;
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    let revertLetter = str[i];
    if (str[i] === str[i].toUpperCase()) {
      revertLetter = revertLetter.toLowerCase();
    }
    if (str[i] === str[i].toLowerCase()) {
      revertLetter = revertLetter.toUpperCase();
    }
    newStr += revertLetter;
  }
  return newStr;
